...
/datum/admins/proc/fax_panel(mob/living/user)
	var/html = "<A align='right' href='?src=[REF(user)];refreshfaxpanel=1'>Refresh</A>"
	html += "<A align='right' href='?src=[REF(user)];AdminFaxCreate=1;faxtype=Administrator'>Create Fax</A>"
...
//			html += "<td>[ADMIN_PP(user)]</td>"
//		else
//			html += "<td>Unknown</td>"
		html += "<td><A align='right' href='?src=[REF(user)];AdminFaxView=\ref[A.message]'>View</A></td>"
		if(!A.reply_to)
			if(A.from_department == "Administrator")
				html += "<td>N/A</td>"
			else
				html += "<td><A align='right' href='?src=[REF(user)];AdminFaxCreate=\ref[A.sent_by];originfax=\ref[A.origin];faxtype=[A.to_department];replyto=\ref[A.message]'>Reply</A></td>"
			html += "<td>N/A</td>"
		else
			html += "<td>N/A</td>"
			html += "<td><A align='right' href='?src=[REF(user)];AdminFaxView=\ref[A.reply_to]'>Original</A></td>"
		html += "</tr>"
	html += "</table>"
	html += "</div>"
...
//		else
//			html += "<td>Unknown</td>"
//		html += "<td><A align='right' href='?src=[REF(user)];AdminFaxView=\ref[F.message]'>View</A></td>"
//		html += "</tr>"
	html += "</table>"
	html += "</div>"
