...
/obj/machinery/photocopier/faxmachine/ui_data(mob/user)
	...
	if(!data["authenticated"])
		...
		data["network"] = "ERR*?*%!*"
	data["paper"] = faxitem ? faxitem.name : FALSE
	data["paperinserted"] = faxitem ? TRUE : FALSE
...
//					user.dropItemToGround()
		...
		if("rename") // rename the item that is currently in the fax machine
			if(paper_copy)
				var/n_name = sanitize(copytext(input(usr, "What would you like to label the fax?", "Fax Labelling", faxitem.name)  as text, 1, MAX_MESSAGE_LEN))
				if((paper_copy && paper_copy.loc == src && usr.stat == 0))
					if(istype(faxitem, /obj/item/paper))
						faxitem.name = "[(n_name ? text("[n_name]") : initial(faxitem.name))]"
						faxitem.desc = "This is a paper titled '" + faxitem.name + "'."
					else if(istype(faxitem, /obj/item/photo))
						faxitem.name = "[(n_name ? text("[n_name]") : "photo")]"
//					else if(istype(faxitem, /obj/item/paper_bundle))
//						faxitem.name = "[(n_name ? text("[n_name]") : "paper")]"
			...
			if(is_authenticated)
				...
				if(syndie_restricted)
					...
					for(var/obj/machinery/photocopier/faxmachine/F in GLOB.allfaxes)
						if(EMAGGED)//we can contact emagged faxes on the station
		...
		if("send") // actually send the fax
			if(!paper_copy || !is_authenticated || !destination)
				return
			if(stat & (BROKEN|NOPOWER))
				return
...
/obj/machinery/photocopier/faxmachine/proc/receivefax(obj/item/incoming)
	...
	playsound(loc, 'modular_splurt/sound/effects/printer_dotmatrix.ogg', 50, 1)
...
//		bundle_copy(incoming) //*WIP* Paper bundles soon tm
	else
		return FALSE
...
/obj/machinery/photocopier/faxmachine/proc/send_admin_fax(mob/sender, destination)
	...
	if(!(istype(paper_copy, /obj/item/paper) || /*istype(bundle_copy, /obj/item/paper_bundle) ||*/ istype(photo_copy, /obj/item/photo)))
		visible_message("[src] beeps, \"Error transmitting message.\"")
		return
	...
	switch(destination)
		if("Central Command")
			message_admins(sender, "CENTCOM FAX", destination, faxitem, "#006100")
		if("Syndicate")
			message_admins(sender, "SYNDICATE FAX", destination, faxitem, "#DC143C")
	for(var/obj/machinery/photocopier/faxmachine/F in GLOB.allfaxes)
		if(F.department == destination)
			F.receivefax(faxitem)
	visible_message("[src] beeps, \"Message transmitted successfully.\"")
...
/obj/machinery/photocopier/faxmachine/proc/message_admins(mob/sender, faxname, faxtype, obj/item/sent, font_colour="#9A04D1")
	var/msg = "<span class='boldnotice'><font color='[font_colour]'>[faxname]: </font> [key_name_admin(sender)] | REPLY: (<A HREF='?_src_=holder;[faxname == "SYNDICATE FAX" ? "SyndicateReply" : "CentcommReply"]=[REF(sender)]'>RADIO</A>) (<a href='?_src_=holder;AdminFaxCreate=\ref[sender];originfax=\ref[src];faxtype=[faxtype];replyto=\ref[sent]'>FAX</a>) ([ADMIN_SM(sender)]) | REJECT: (<A HREF='?_src_=holder;FaxReplyTemplate=[REF(sender)];originfax=\ref[src]'>TEMPLATE</A>) </span>: Receiving '[sent.name]' via secure connection... <a href='?_src_=holder;AdminFaxView=\ref[sent]'>view message</a>"
