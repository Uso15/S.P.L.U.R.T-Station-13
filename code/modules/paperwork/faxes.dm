...
/datum/admins/proc/fax_panel(var/mob/living/user)
	var/html = "<A align='right' href='?src=[REF(user)];[HrefToken(TRUE)];refreshfaxpanel=1'>Refresh</A>"
	html += "<A align='right' href='?src=[REF(user)];[HrefToken(TRUE)];AdminFaxCreate=1;faxtype=Custom'>Create Fax</A>"
	...
	for(var/thing in GLOB.adminfaxes)
		...
		if(A.sent_by)
			...
			html += "<td>Unknown</td>"
		html += "<td><A align='right' href='?src=[REF(user)];[HrefToken(TRUE)];AdminFaxView=[REF(A.message)]'>View</A></td>"
		if(!A.reply_to)
			if(A.from_department == "Administrator")
				html += "<td>N/A</td>"
			else
				html += "<td><A align='right' href='?src=[REF(user)];[HrefToken(TRUE)];AdminFaxCreate=[REF(A.sent_by)];originfax=[REF(A.origin)];faxtype=[A.to_department];replyto=[REF(A.message)]'>Reply</A></td>"
			html += "<td>N/A</td>"
		else
			html += "<td>N/A</td>"
			html += "<td><A align='right' href='?src=[REF(user)];[HrefToken(TRUE)];AdminFaxView=[REF(A.reply_to)]'>Original</A></td>"
		html += "</tr>"
	html += "</table>"
	html += "</div>"
	...
	for(var/thing in GLOB.faxes)
		...
		if(F.sent_by)
			...
			html += "<td>Unknown</td>"
		html += "<td><A align='right' href='?src=[REF(user)];[HrefToken(TRUE)];AdminFaxView=[REF(F.message)]'>View</A></td>"
		html += "</tr>"
	html += "</table>"
	html += "</div>"
